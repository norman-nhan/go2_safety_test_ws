"""Make a standing Go2 nod its fixed head up and down."""
...
# The Go2 head is fixed to its torso, so body pitch creates the head motion.
HEAD_PITCH_RAD = math.radians(10.0)
SHAKE_HZ = 1.5
SHAKE_SECONDS = 4.0
UPDATE_HZ = 15.0
...
async def shake_head(connection):
    """Oscillate torso pitch to move the fixed head up and down."""
    ...
    while loop.time() - start < SHAKE_SECONDS:
        ...
        pitch = HEAD_PITCH_RAD * math.sin(math.tau * SHAKE_HZ * elapsed)
        await set_body_pose(connection, pitch=pitch)
...
async def main():
    ...
    try:
        ...
        print("Shaking head up and down...")
